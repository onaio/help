...
This means transferring project ownership from one account to another. When you transfer a project, the project and all of its forms and data will be transferred  to the new account and the original account will lose ownership of the project. A user can only transfer a project out of an individual account if they are the project admin.  When transferring the project to an org account, the user must also be an admin or project manager on the org account. To transfer a project out of an org account, the user must be an admin in the org.

The following table describes conditions for transferring project ownership out/in an organization and individual accounts: 


| Project ownership transfer condition                         | Permission level                                       | 
| ------------------------------------------------------------ |------------------------------------------------------  | 
| Transfer a project out of an individual account  | A user must be the project admin (project creator).| 
| Transfer a project to an individual account      | No permissions required, when transferring the project ownership to another individual account you do not need any permission level.                                          |  
| Transfer a project out of an organization account| A user must be an [Org member](https://help.ona.io/guides/organizations/#organization-members) with the Org Admin role or a project admin in one of the [organization projects](https://help.ona.io/guides/organizations/#organization-projects).                     |   
| Transfer a project  into an organization account| No permissions required. However, you'll no longer have access to the project unless you're an org member. |   
...
> 
**Note:**<br/> Individual accounts have only one admin — the account the project lives on, which for new projects is the creator of the project. To have multiple admins on a single project, create an [organization project](http://help.ona.io/guides/organizations/#creating-org-projects).

>
**Note:**<br/> To accommodate legacy ODKCollect users the XML representation of private forms is available at a URL that is not password protected.
